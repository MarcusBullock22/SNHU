...
void HashTable::Insert(Bid bid)
{
	...
	// search for bid for requested value

	Node* prevNode = &(myNode.at(key));

	if (prevNode == nullptr) {
		Node* nextNode = new Node(bid, key);
		myNode.insert(myNode.begin() + key, (*nextNode));
	}
	else {
		// for when bid is found
...
void HashTable::PrintAll()
{
	...
	for (unsigned int i = 0; i < myNode.size(); ++i)
	{
		displayBid(myNode[i].bid);
	}
}
